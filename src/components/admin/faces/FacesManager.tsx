...
export default function FacesManager({
  ...
  initialFaces: FaceRecord[];
  initialStudents: FaceStudentOption[];
}) {
  const [faces, setFaces] = useState(initialFaces),
    [students, setStudents] = useState(initialStudents),
    [query, setQuery] = useState(""),
    [status, setStatus] = useState(""),
    [enrollStudent, setEnrollStudent] = useState<number | null | undefined>(
      undefined,
    ),
    [viewing, setViewing] = useState<FaceRecord | null>(null),
  ...
  const filtered = useMemo(() => {
    ...
    return faces.filter(
      (face) =>
        (!needle ||
          [face.studentCode, face.studentName, face.className].some((item) =>
            item.toLocaleLowerCase("th").includes(needle),
          )) &&
        (!status || face.status === status),
    );
  }, [faces, query, status]);
  ...
  async function reload(message?: string) {
    ...
    if (!response.ok || !data.faces || !data.students)
  ...
  async function toggle(face: FaceRecord) {
    setBusyId(face.studentId);
    try {
      const next = face.status === "READY" ? "INACTIVE" : "READY",
        response = await fetch("/api/faces", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ studentId: face.studentId, status: next }),
        }),
        data = (await response.json()) as { message?: string };
      if (!response.ok)
  ...
  return (
    <main className="admin-content">
      <div className="page-intro">
        <div>
          <h2>ข้อมูลใบหน้า</h2>
          <p>จัดการภาพอ้างอิงและ Face Embedding สำหรับการเช็คชื่อ</p>
        </div>
        <div className="face-page-actions">
          <button
      ...
      <div className="admin-filters">
        <label>
          <Search size={18} />
          <input
            aria-label="ค้นหาข้อมูลใบหน้า"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="ค้นหารหัส ชื่อนักเรียน หรือห้องเรียน"
          />
        </label>
        <select
        ...
        >
          ...
          <option value="NEEDS_IMAGES">ควรเพิ่มรูป</option>
        </select>
        <button
          className="admin-button secondary"
          onClick={() => {
      ...
      <section className="dashboard-card admin-table-card">
        <div className="card-head">
          <div>
            <h2>ข้อมูลใบหน้าที่ลงทะเบียน</h2>
            <p>
              แสดง {filtered.length} จาก {faces.length} คน
            </p>
          </div>
        ...
        <div className="admin-data-wrap">
          <table>
            ...
            <tbody>
              {filtered.map((face) => (
                <tr key={face.studentId}>
                  <td>
                    {face.sampleIds[0] ? (
                      <Image
                        className="face-table-image"
                        src={`/api/faces/image?id=${face.sampleIds[0]}`}
                        width={48}
                        height={48}
                        unoptimized
                        alt={face.studentName}
                      />
                    ) : (
                      <span className="face-table-empty">
                        <ScanFace size={20} />
                      </span>
                    )}
                  </td>
                  <td>
                    <PersonCell
                      initials={face.studentName
                        .replace("เด็กชาย", "")
                        .replace("เด็กหญิง", "")
                        .slice(0, 2)}
                      name={face.studentName}
                      detail={face.studentCode}
                    />
                  </td>
                  <td>{face.className}</td>
                  <td>{face.photoCount} รูป</td>
                  <td>
                    <Badge
                      tone={
                        face.status === "READY"
                    ...
                    >
                      {STATUS_LABEL[face.status]}
                    </Badge>
                  </td>
                  <td>{face.updatedAt}</td>
                  <td>
                    <div className="table-actions">
                      <button onClick={() => setViewing(face)} title="ดู">
                        <Eye size={16} />
                        <span>ดู</span>
                      </button>
                      <button
                        onClick={() => setEnrollStudent(face.studentId)}
                        title="แก้ไข"
                      >
                        <Pencil size={16} />
                        <span>แก้ไข</span>
                      ...
                      <button
                        disabled={busyId === face.studentId}
                        onClick={() => toggle(face)}
                        title={
                          face.status === "READY" ? "ปิดใช้งาน" : "เปิดใช้งาน"
                        }
                      >
                        <Power size={16} />
                        <span>{face.status === "READY" ? "ปิด" : "เปิด"}</span>
                      </button>
                      <button
                        className="danger"
                        disabled={busyId === face.studentId}
                ...
                <tr>
                  <td colSpan={7}>
                    <div className="subject-empty">
                      <span>ไม่พบข้อมูลใบหน้า</span>
                      <p>กด “ลงทะเบียนใบหน้า” เพื่อเพิ่มข้อมูล</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {viewing && (
        ...
        >
          <section className="subject-modal face-gallery-modal">
            <header>
              <div>
                <h2>ภาพอ้างอิงของ {viewing.studentName}</h2>
                <p>
                  {viewing.studentCode} · {viewing.className} · ลงทะเบียน{" "}
            ...
            <div className="face-view-grid">
              {viewing.sampleIds.map((id, index) => (
                <figure key={id}>
                  <Image
                    src={`/api/faces/image?id=${id}`}
                    width={220}
                    height={220}
                    unoptimized
                    alt={`ภาพอ้างอิง ${index + 1}`}
                  />
