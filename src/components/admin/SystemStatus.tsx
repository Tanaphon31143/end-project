...
export function SystemStatus() {
  ...
  useEffect(() => {
    queueMicrotask(() =>
      setCamera(Boolean(navigator.mediaDevices?.getUserMedia) ? "ok" : "error"),
    );
    fetch("/api/db-health")
  ...
  return (
    <section className="dashboard-card status-card">
      <div className="card-head">
        <div>
          <h2>สถานะระบบ</h2>
          <p>ตรวจสอบจากอุปกรณ์และบริการปัจจุบัน</p>
        </div>
      </div>
      <div className="system-list">
        {systems.map(({ label, Icon, state }) => {
          const checking = state === "checking";
          const ok = state === "ok";
          return (
            <div className="system-status-row" key={label}>
              ...
              <span
                className={
                  ok ? "normal" : checking ? "status-checking" : "status-error"
                }
              >
                <i />
                {checking ? "ตรวจสอบ" : ok ? "ปกติ" : "ผิดปกติ"}
              </span>
            </div>
          );
        })}
      </div>
      <Link className="view-all" href="/admin/settings">
        ดูสถานะทั้งหมด <ChevronRight size={15} />
      </Link>
    </section>
  );
}
