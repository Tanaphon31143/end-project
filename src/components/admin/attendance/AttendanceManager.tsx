...
export function AttendanceManager({
  ...
  const selectedStudent = initialData.students.find(
    (item) => item.id === Number(form.studentId),
  );
  const availableSubjects = initialData.subjects.filter(
    (item) =>
      !selectedStudent ||
      !item.classroomId ||
      item.classroomId === selectedStudent.classroomId,
  );
  ...
  async function openDetail(id: number) {
    ...
    setMessage("");
    try {
      const response = await fetch(`/api/attendance?recordId=${id}`, {
        cache: "no-store",
      });
      ...
      setDetail(data);
      setEditStatus(data.record.status);
      setEditNote("");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "เกิดข้อผิดพลาด");
    } finally {
  ...
  return (
    <>
      ...
      <section className="attendance-filter dashboard-card">
        ...
        <label>
          ...
          >
            ...
            {initialData.classrooms.map((item) => (
              <option key={item.id} value={item.id}>
                {item.name}
              </option>
            ))}
      ...
      <div className="mini-summary attendance-summary">
              ...
              {records.map((row) => (
                <tr key={row.id}>
                  <td>
                    ...
                    {row.attendanceDate.split("-").reverse().join("/")}
                  </td>
                  <td>
                    <div className="attendance-person">
                      <span>{row.studentName.slice(0, 2)}</span>
                      <div>
                        <b>{row.studentName}</b>
                  ...
                  <td>
                    <b>{row.subjectCode}</b>
                    <small className="subject-name">{row.subjectName}</small>
                  </td>
                  <td>
                    {row.checkInTime ? (
                      <>
                        <Clock3 size={14} /> {row.checkInTime}
                      </>
                    ) : (
                      "-"
                    )}
                  ...
                  <td>
                    <span className={`data-badge ${tones[row.status]}`}>
                      {labels[row.status]}
                    </span>
                  </td>
                  <td>
                ...
                <tr>
                  <td colSpan={8}>
                    <div className="subject-empty">
                      <span>ไม่พบข้อมูลการเข้าเรียน</span>
                      <p>ลองเปลี่ยนเงื่อนไขการค้นหา หรือเพิ่มข้อมูลใหม่</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      {addOpen && (
        ...
        >
          <section
            className="admin-modal attendance-modal"
            role="dialog"
            aria-modal="true"
          >
            <header>
              <div>
                <h2>เพิ่มข้อมูลการเข้าเรียน</h2>
                <p>รายการที่เพิ่มเองจะมีค่าความมั่นใจเป็น “บันทึกเอง”</p>
              </div>
              <button onClick={() => setAddOpen(false)} aria-label="ปิด">
                <X />
              </button>
            </header>
              ...
              <label>
                ...
                >
                  ...
                  {initialData.students.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.code} · {item.name} · {item.className}
                    </option>
                  ))}
                </select>
              </label>
            ...
            <footer>
              ...
              <button
                className="admin-button primary"
                disabled={loading || !form.studentId || !form.subjectId}
      ...
      {detail && (
        ...
        >
          <section
            className="admin-modal attendance-modal detail-modal"
            role="dialog"
            aria-modal="true"
          >
            <header>
              <div>
                ...
                <p>รายการ #{detail.record.id}</p>
              </div>
              <button onClick={() => setDetail(null)} aria-label="ปิด">
                <X />
              </button>
            </header>
            <div className="attendance-detail-grid">
              <div>
                <UserRound />
                <span>นักเรียน</span>
                <b>{detail.record.studentName}</b>
                <small>
                  {detail.record.studentCode} · {detail.record.className}
                </small>
              </div>
              <div>
                <CalendarDays />
                ...
                <b>
                  {detail.record.attendanceDate.split("-").reverse().join("/")}
                </b>
                <small>
                  {detail.record.checkInTime || "ไม่มีเวลาเช็คชื่อ"}
                </small>
              </div>
              <div>
                <Clock3 />
                <span>รายวิชา</span>
                <b>{detail.record.subjectCode}</b>
                <small>{detail.record.subjectName}</small>
              </div>
              <div>
                <History />
                <span>แหล่งข้อมูล</span>
                <b>
                  {detail.record.confidence == null
                    ? "ผู้ดูแลบันทึก"
                    : `Face Recognition ${Number(detail.record.confidence).toFixed(2)}%`}
                </b>
                <small>สร้างเมื่อ {detail.record.createdAt}</small>
              </div>
            </div>
            <div className="attendance-edit">
              <h3>
                <Pencil size={17} /> แก้ไขสถานะ
              </h3>
              <div>
                <label>
                  <span>สถานะใหม่</span>
                  <select
                    value={editStatus}
                    onChange={(event) =>
                      setEditStatus(event.target.value as AttendanceStatus)
                    }
                  >
                    {Object.entries(labels).map(([value, label]) => (
                      <option key={value} value={value}>
                ...
                <label>
                  ...
                  <input
                    ...
                    placeholder="ระบุเหตุผลที่แก้ไข"
                  />
                </label>
                <button
                  className="admin-button primary"
                  disabled={loading || editStatus === detail.record.status}
            ...
            <div className="audit-history">
              <h3>
                <History size={17} /> ประวัติผู้แก้ไข
              </h3>
                    ...
                    <div>
                      <b>
                        {audit.action === "ADD"
                          ...
                          : `เปลี่ยนจาก ${audit.oldStatus ? labels[audit.oldStatus] : "-"} เป็น ${labels[audit.newStatus]}`}
                      </b>
                      <p>
                        โดย {audit.adminName} · {audit.createdAt}
                      </p>
                      {audit.note && <small>หมายเหตุ: {audit.note}</small>}
                    </div>
                  </article>
                ))
              ) : (
                <p className="audit-empty">
                  ยังไม่มีประวัติการแก้ไข (รายการเก่าก่อนเปิดใช้ Audit Log)
                </p>
              )}
            </div>
          </section>
