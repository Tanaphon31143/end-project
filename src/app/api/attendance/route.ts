...
export async function GET(request: Request) {
  ...
  if (search) {
    where.push("(st.student_code LIKE ? OR st.full_name LIKE ?)");
...
export async function POST(request: Request) {
  ...
  if (
    ...
    subjectId < 1 ||
    !datePattern.test(attendanceDate) ||
    !isStatus(status)
  ) {
    return Response.json(
      { message: "กรุณากรอกข้อมูลการเข้าเรียนให้ครบถ้วน" },
      { status: 400 },
    );
  }
  if (
    ...
    return Response.json(
      { message: "กรุณาระบุเวลาเข้าเรียนให้ถูกต้อง" },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    >(
      `SELECT st.id studentId,st.class_id studentClassId,sb.classroom_id subjectClassId FROM students st CROSS JOIN subjects sb WHERE st.id=? AND sb.id=? AND st.status='ACTIVE'`,
      [studentId, subjectId],
    );
    ...
    if (!relation) {
      ...
      return Response.json(
        { message: "ไม่พบนักเรียนหรือรายวิชาที่เลือก" },
        { status: 404 },
      );
    }
    if (
      relation.subjectClassId &&
      relation.studentClassId !== relation.subjectClassId
    ) {
      await connection.rollback();
      return Response.json(
        { message: "นักเรียนไม่ได้อยู่ในห้องของรายวิชานี้" },
        { status: 400 },
      );
    }
    const dateTime =
      status === "PRESENT" || status === "LATE"
        ? `${attendanceDate} ${checkInTime}:00`
        : null;
    const [result] = await connection.execute<ResultSetHeader>(
      "INSERT INTO attendance_records(student_id,subject_id,attendance_date,check_in_time,status,confidence) VALUES(?,?,?,?,?,NULL)",
      [studentId, subjectId, attendanceDate, dateTime, status],
    );
    await connection.execute(
      "INSERT INTO attendance_record_audits(attendance_record_id,admin_id,action,old_status,new_status,note) VALUES(?,?,'ADD',NULL,?,?)",
      [result.insertId, admin.id, status, note || null],
    );
    await connection.commit();
    return Response.json(
      { id: result.insertId, message: "เพิ่มข้อมูลการเข้าเรียนแล้ว" },
      { status: 201 },
    );
  } catch (error) {
    await connection.rollback();
    if ((error as { code?: string }).code === "ER_DUP_ENTRY")
      return Response.json(
        { message: "นักเรียนมีข้อมูลในรายวิชาและวันที่นี้แล้ว" },
        { status: 409 },
      );
    console.error("Create attendance failed", error);
    return Response.json(
      { message: "ไม่สามารถเพิ่มข้อมูลการเข้าเรียนได้" },
      { status: 500 },
    );
  } finally {
    connection.release();
...
export async function PATCH(request: Request) {
  ...
  const id = Number(body.id),
    ...
    note = String(body.note || "")
      .trim()
      .slice(0, 500);
  if (!Number.isInteger(id) || id < 1 || !isStatus(status))
    return Response.json({ message: "ข้อมูลสถานะไม่ถูกต้อง" }, { status: 400 });
  const connection = await db.getConnection();
  try {
    await connection.beginTransaction();
    const [records] = await connection.execute<
      (RowDataPacket & { status: AttendanceStatus })[]
    >("SELECT status FROM attendance_records WHERE id=? FOR UPDATE", [id]);
    ...
    await connection.execute(
      "INSERT INTO attendance_record_audits(attendance_record_id,admin_id,action,old_status,new_status,note) VALUES(?,?,'STATUS_UPDATE',?,?,?)",
      [id, admin.id, oldStatus, status, note || null],
    );
    await connection.commit();
    ...
    return Response.json(
      { message: "ไม่สามารถแก้ไขสถานะได้" },
      { status: 500 },
    );
  } finally {
    connection.release();
